...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    newdf=pd.DataFrame()
    newdf['NDC_Code']=ndc_df['NDC_Code']
    newdf['Non-proprietary Name']=ndc_df['Non-proprietary Name']

    code_dict=dict(newdf.values)
    code_dict.update({np.nan:np.nan})
    df['generic_drug_name'] = df['ndc_code'].apply(lambda x: code_dict[x])
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    uniqueval=df['patient_nbr'].unique()
    total=len(uniqueval)
    n_train=int(0.6*total)
    n_valid=int(0.2*total)
    n_test = total - n_train - n_valid

    train = df[df['patient_nbr'].isin(uniqueval[:n_train])].reset_index(drop=True)
    validation = df[df['patient_nbr'].isin(uniqueval[n_train:n_train+n_valid])].reset_index(drop=True)
    test = df[df['patient_nbr'].isin(uniqueval[n_train+n_valid:])].reset_index(drop=True)
